...
def criar_datasets(n_usuarios, n_filmes,porcentagem_treino):
	...
	n_ratings = n_usuarios * n_filmes
	limite_treino = n_ratings * (porcentagem_treino/100)
	indices_teste = []

	texto = open("ml-100k/u.data","r")
	# texto = open("ml-100k/fake.data","r")
	linhas = texto.readlines()

	# cria matriz de permutacao aleatoria de 0 a n_ratings-1
	arranjo = np.random.permutation(n_ratings).reshape((n_usuarios,n_filmes))
	...
	for linha in linhas:
		...
		if arranjo[i_usuario, i_filme] < limite_treino:
...
def prever_nota(d_treino, usuario, filme, vizinhos, media_global, alpha,media_filmes,similaridades, media_usuarios):
	...
	for v in vizinhos:
		if d_treino[v,filme] > 0.1:
			num += float(similaridades[usuario,v]*(d_treino[v,filme] - media_usuarios[v]))
			den += float(similaridades[usuario,v])
...
def knn(d_treino, d_teste,n_usuarios, n_filmes, indices_teste, tipo_similaridade):
