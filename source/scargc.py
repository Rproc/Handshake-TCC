...
def newScargc(dataset, data_labeled, d_treino, l_train, stream, l_stream, pool_size, num_clusters, n_features, k):
    ...
    if num_clusters == num_class:
        ...
        centroid_past_lab, a, b = knn_classify(d_treino[:,:-1], l_train, np.reshape(centroid_past[0,:], (-1, (n_features - 1))))#KNeighborsClassifier(n_neighbors=k)
        # KNN.fit(d_treino[:,:-1], l_train)
        # centroid_past_lab = []
        # centroid_past_lab = KNN.predict(np.reshape(centroid_past[0,:], (-1, (n_features - 1))))

        for core in range(1, centroid_past.shape[0]):
            pred, a, b = knn_classify(d_treino[:,:-1], l_train, np.reshape(centroid_past[core,:], (-1, (n_features - 1))))
            # KNN.predict(np.reshape(centroid_past[core,:], (-1, (n_features - 1))))
            centroid_past_lab = np.vstack([centroid_past_lab, pred])
    ...
    for i in range(0, len(stream)):
        ...
        predicted, a, b = knn_classify(d_treino[:,:-1], l_train, x)
        ...
        if len(pool) == pool_size:
            ...
            clab, a, nearest = knn_classify(centroid_past[:,:-1], centroid_past[:,-1], centroid_cur[0,:])
            ...
            intermed = np.hstack( [np.median(np.vstack([nearest, centroid_cur[0,:]]), axis=0), clab])
            # centroid_label = clab[0]
            # print('before loop', intermed)

            for p in range(1, centroid_cur.shape[0]):

                clab, a, nearest = knn_classify(centroid_past[:,:-1], centroid_past[:,-1], centroid_cur[p,:])

                aux = np.hstack([np.median(np.vstack([nearest, centroid_cur[p,:]]), axis=0), clab])
                intermed = np.vstack([intermed, aux])
                centroid_label = np.vstack([centroid_label, clab])

            centroid_cur = np.column_stack([centroid_cur, centroid_label])
            centroid_past = intermed

            # KNN.fit( np.vstack([centroid_cur[:,:-1], centroid_past[:,:-1]]), np.hstack([centroid_cur[:,-1], centroid_past[:,-1]]) )
            # pred_all = KNN.predict(pool[:,0:-1])pool_data[p,0:-1]

            pred, a, b = knn_classify(np.vstack([centroid_cur[:,:-1], centroid_past[:,:-1]]), np.hstack([centroid_cur[:,-1], centroid_past[:,-1]]), pool[0,0:-1])
            new_pool = np.hstack([pool[0,0:-1], pred])

            for p in range(1, pool.shape[0]):

                pred, a, b = knn_classify(np.vstack([centroid_cur[:,:-1], centroid_past[:,:-1]]), np.hstack([centroid_cur[:,-1], centroid_past[:,-1]]), pool[p,0:-1])

                new_pool = np.vstack([new_pool, np.hstack([pool[p,0:-1], pred])])
